...
def check_user(func):
    def inner(message):
        if user_is_allowed(message.from_user):
            func(message)
        else:
            bot.send_message(message.from_user.id, "Вы не зарегистрированы, дождитесь проверки администратора.")
    return inner
